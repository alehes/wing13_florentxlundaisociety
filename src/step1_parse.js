...
const PARSE_SYSTEM = `You will be extracting structured information from nurse emails and formatting the data as JSON objects. Each email may contain information about one or more patients/jobs.

For each patient/job mentioned in the email, extract these fields:
...
export async function parseJobsFromEmail(emailText) {
  const message = await client.messages.create({
    ...
    messages: [{
      role: "user",
      content: `Here is the email text to process:\n\n<email>\n${emailText}\n</email>`,
    }],
  });

  if (message.stop_reason === "max_tokens") {
    throw new Error("Claude response truncated in step1 — increase max_tokens");
  }
  ...
  try {
    ...
    throw new Error(`Failed to parse JSON from step1 response: ${err.message}\nRaw: ${raw.slice(0, 200)}`);
  }
}